...
def get_relation_insts(vid, tet):
    with open(os.path.join('data/chaos/annotations', tet, vid)) as f:
        anno = json.load(f)
        f.close()
    sub_objs = dict()
...
def eval_rel_results(args, all_results, output_dir, topk=100, is_gt_traj=False):
    ...
    if args.sharp_frame:
        ...
        if os.path.exists(os.path.join('./data/chaos', 'sharp_key_frame_set.pkl')):
            ...
            with open(os.path.join('./data/chaos', 'sharp_key_frame_set.pkl'), 'wb') as f:
    ...
    for prd_k in prd_k_set:
        ...
        for im_i, res in enumerate(tqdm(all_results)):
            ...
            for iN, N in enumerate(ans_N_list):
                ...
                if not os.path.exists(os.path.join('data/chaos/features', suffix_cator, \
                ...
                if res['prd_scores'] is None:
                    ...
                    with open(os.path.join('data/chaos/features', suffix_cator, \
                        file_real_name[:-4], file_real_name[:-4]+'-'+'{:04d}'.format(pstart)+\
        ...
        print('Saving transform_chaos_results dets...')
        topk_dets_f = os.path.join(output_dir, 'transform_chaos_results.pkl')
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="transform_chaos_results")
    ...
    parser.add_argument('--segment_groundtruth', 
            default="data/chaos/annotations/test_seg_gt.json", 
    ...
    if not os.path.exists(os.path.join(args.output_dir, 'transform_chaos_results.pkl')):
        ...
        with open(os.path.join(args.output_dir, 'transform_chaos_results.pkl'), 'rb') as f:
    ...
    if args.is_seg_test:
        if os.path.exists(args.segment_groundtruth):
            ...
            init_path = os.path.join('data/chaos/annotations', tet)
